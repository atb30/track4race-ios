...
export default function QuickPoiDialog({ open, onOpenChange, snapshotKm, maximumKm, poiTypes, saving, error, onSave }) {
  ...
  const submit = async (event) => {
    ...
    if (!name.trim()) return setValidationError(en ? 'Enter a name for the POI.' : 'Escribe un nombre para el POI.');
    if (!poiTypeId) return setValidationError(en ? 'Select a category.' : 'Selecciona una categoría.');
    if (!Number.isFinite(parsedDistance) || parsedDistance < 0 || parsedDistance > maximumKm) {
      return setValidationError(en ? `The kilometer point must be between 0 and ${Number(maximumKm || 0).toFixed(3)} km.` : `El PK debe estar entre 0 y ${Number(maximumKm || 0).toFixed(3)} km.`);
    }
    setValidationError('');
      ...
      <Dialog.Portal>
        ...
        <Dialog.Content className="fixed z-[6001] left-1/2 top-1/2 w-[calc(100%_-_24px)] max-w-md -translate-x-1/2 -translate-y-1/2 rounded-2xl border border-blue-100 bg-white p-5 shadow-2xl">
          <Dialog.Title className="flex items-center gap-2 text-lg font-bold text-slate-900">
            ...
            {en ? 'Add POI' : 'Añadir POI'}
          </Dialog.Title>
          <Dialog.Description className="mt-2 text-sm text-slate-500">
            {en ? 'The exact point where you pressed the button was saved. You can adjust the kilometer point before saving.' : 'Se ha memorizado el punto exacto en el que pulsaste el botón. Puedes corregir el PK antes de guardarlo.'}
          </Dialog.Description>
          ...
          <form onSubmit={submit} className="mt-5 space-y-4">
            <div className="space-y-1.5">
              <Label htmlFor="quick-poi-name">{en ? 'Name' : 'Nombre'}</Label>
            ...
            <div className="space-y-1.5">
              <Label>{en ? 'Category' : 'Categoría'}</Label>
              <Select value={poiTypeId} onValueChange={setPoiTypeId}>
                <SelectTrigger><SelectValue placeholder={en ? 'Select a category' : 'Selecciona una categoría'} /></SelectTrigger>
            ...
            <div className="space-y-1.5">
              <Label htmlFor="quick-poi-km">{en ? 'Kilometer point' : 'Punto kilométrico'} (PK)</Label>
            ...
            <div className="flex gap-3 pt-2">
              <Button type="button" variant="outline" className="flex-1" disabled={saving} onClick={() => onOpenChange(false)}>{en ? 'Cancel' : 'Cancelar'}</Button>
              <Button type="submit" className="flex-1 bg-blue-600 hover:bg-blue-700" disabled={saving || poiTypes.length === 0}>
                <Save className="mr-2 h-4 w-4" />{saving ? (en ? 'Saving…' : 'Guardando…') : (en ? 'Save' : 'Guardar')}
              </Button>
            </div>
          </form>
        </Dialog.Content>
